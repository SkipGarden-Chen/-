...
- `prepare_yolo_dataset.py`：从无人机视频抽帧，创建 YOLO 数据集目录和 `data.yaml`。
- `train_yolo.py`：训练 YOLO 地质现象检测模型。
- `predict_yolo.py`：用训练好的模型对图片、文件夹或视频推理。
...
```powershell
python .\src\prepare_yolo_dataset.py --dataset .\data\yolo_dataset
```
...
抽帧后，使用 LabelImg、CVAT、Roboflow 或 makesense.ai 标注图片，并整理为：
...
如果没有安装 `ultralytics` 或模型文件不存在，M1 会自动回退到 `observations.csv` 中的 `yolo_result` 字段，保证剖面图流程仍然可以跑通。

当前默认类别见 [data/yolo_dataset/data.yaml](data/yolo_dataset/data.yaml)。
